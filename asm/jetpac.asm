...
RocketLanding:
  ...
  cp $b7                  ;
  jr c,UpdateRocketColour ; Update rocket colour if not (and RET)
; Rocket has landed!
  ld (ix+$00),$09         ; Set Rocket "move" state to default (on pad)
  call PlayerInit         ; Initialise the player state
  jr UpdateRocketColour   ; Update rocket colour (and RET)
...
RocketUpdate:
  ...
  dec e                    ; Update rocket colour if not zero (and RET)
  jr nz,UpdateRocketColour ;
  ld a,(ix+$05)           ; Update rocket colour if fuel pod counter < 6 (and
  cp $06                  ; RET)
  ...
  inc (hl)                ;
  jp DisplayPlayerLives   ; Display player lives (and RET)
...
; return. Used by the routines at RocketTakeoff, RocketLanding and
; RocketUpdate.
;
; Input:IX Rocket object.
UpdateRocketColour:
...
;
; Used by the routines at SfxJetmanDeath and SfxEnemyDeath.
;
; Input:IX The explosion SFX params array.
SfxFinishReturn:
...
ResetModifiedInFrame:
  ...
  pop ix                  ; Restore jump table address, saved during frame
                          ; update
...
;
; Used by the routines at SquidgyAlienUpdate, UFOUpdate, SphereAlienUpdate and
; CrossedShipUpdate.
;
; Input:IX Alien object.
DrawAlien:
...
; routine at BufferCopySpriteData.
;
; Input:B Loop counter - sprite height.
...
BufferCopyRocket_0:
  ...
  ld b,a                  ; A starts at $00, then returns value from
                          ; BufferCopyRocketSpriteData
  ld c,$01                ; C=sprite count
  call BufferCopyRocketSpriteData ; HL=pointer to sprite pixel data
  ld a,b
  pop bc                  ; Restore loop counter
...
BufferCopyRocketSpriteData:
  ...
  jr BufferCopySpriteData ; Perform the copy
...
;
; Used by the routine at BufferCopySpriteData.
;
;  Input:HL Offset address in sprite lookup table.
; Output:HL Pointer to pixel data block.
BufferCopySprite:
...
BufferCopySpriteData:
  ...
  jr c,BufferCopySpriteData_0 ;
  ld a,$10                ; else height=16 pixels
BufferCopySpriteData_0:
  ...
  ld a,(rocket_mod_attached) ; A=Rocket build state - only $00 or $04?
  add a,b
  ld b,a
  dec c                   ; Decrement the sprite counter
  jr nz,BufferCopySprite  ; Fetch next sprite address
  ret
...
ActorGetSpriteAddress:
  ...
  set 3,c                 ; elseact set bit-3 of X position
...
ScreenPosOnePixelAbove:
  dec h                   ; Decrement H to move up one pixel on screen
  ld a,h
  and $07
  cp $07                  ; Has a character line been crossed?
  ret nz                  ; If not, return
  ld a,l                  ; else subtract 32 from L
  sub $20                 ;
  ld l,a                  ;
  and $e0
  cp $e0                  ; Has a new section of the screen been crossed?
  ret z                   ; Return if not
  ld a,h
  add a,$08
  ld h,a
...
;
; Used by the routine at JetmanFlyThrustLeft.
;
; Input:IX Jetman object.
JetmanFlyIncSpdX:
...
;
; Used by the routines at JetmanSetSpdY, JetmanSetZeroSpdY and JetmanDirFlipY.
;
; Input:IX Jetman object.
JetmanFlyVertical:
...
;
; Used by the routines at JetmanSetMoveUp and JetmanHitScreenTop.
;
; Input:IX Jetman object.
JetmanCollision:
...
MaskSprite_1:
  ...
  call ScreenPosOnePixelAbove ; Calculate new pixel position
  pop bc
